...
dados <- read_excel("dataset/dados_cereais.xlsx")
...
mshapiro.test(t(dados[, 4:11]))

# Verificando suposi??o de vari?ncias iguais
boxM(data = dados[,4:11], grouping = dados$Fabricante)

########### Sele??o das vari?veis
# Medidas de qualidade
discPower(variables = dados[,4:11], group = dados$Fabricante)
...
# Ajuste total
fit <- linDA(variables =  dados[, c("Calorias","Proteina","Gordura","Sodio","Fibra","Carboidrato","Acucar","Potassio")], group = dados$Fabricante)

# Ajuste com cross-validaton
fit2 <- linDA(variables =  dados[, c("Calorias","Proteina","Gordura","Sodio","Fibra","Carboidrato","Acucar","Potassio")],
              group = dados$Fabricante, validation = 'crossval')

# Ajuste com amostras treinamento/teste
treinamento <- sample(seq_len(nrow(dados)), size = 32) #75% dos dados
teste <- setdiff(seq_len(nrow(dados)), treinamento)
fit3 <- linDA(variables = dados[, c("Calorias","Proteina","Gordura","Sodio","Fibra","Carboidrato","Acucar","Potassio")], 
              group = dados$Fabricante, validation = 'learntest', learn = treinamento,
              test = teste)
...
classify(fit, 
         newdata = data.frame(
           ...
           Carboidrato = 10.5,
           Acucar = 10,
           Potassio = 70))$pred_class

# Modelo discriminante quadr?tico
discrim_quad <- quaDA(variables = dados[, c("Calorias","Proteina","Gordura","Sodio","Fibra","Carboidrato","Acucar","Potassio")],
                      group = dados$Fabricante)
discrim_quad2 <- quaDA(variables = dados[, c("Calorias","Proteina","Gordura","Sodio","Fibra","Carboidrato","Acucar","Potassio")], 
                       group = dados$Fabricante, validation = 'crossval')
discrim_quad3 <- quaDA(variables = dados[, c("Calorias","Proteina","Gordura","Sodio","Fibra","Carboidrato","Acucar","Potassio")],
                       group = dados$job, validation = 'learntest',  learn = treinamento, test = teste)
