...
dados <- read.csv('dataset/dados-poluicao.csv', sep = ';', dec = ',');glimpse(dados)
...
ajuste1_matriz_covar <- princomp(dados, cor=FALSE)

# Impress?o da vari?ncia explicada
summary(ajuste1_matriz_cor) 
summary(ajuste1_matriz_covar) 

# Scree Plot
plot(ajuste1_matriz_cor, type="lines") 
plot(ajuste1_matriz_covar, type="lines") 
...
ajuste2_covar <- principal(dados, covar = TRUE, nfactors = 2 );ajuste2_covar
