...
```{r import_dataset, echo=FALSE, results="hide", include=FALSE}
library(readr)
library(foreach)
dados <- read_csv("wspc_rlang/dataset/dados_Hair_etal.csv")
```

```{r balanceamento}
dados$size
dados$use
table(dados$size) # 40 karge e 60 small. Isso estaria  desbalanceado
chisq.test(table(dados$size)) # testando se esta desbalanceado
...
```{r post_hoc}
TukeyHSD(result) # somente e necessario quando temos mais de 2 grupos para fazer 
```