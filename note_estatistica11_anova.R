...
if(!require(biotools)){install.packages("biotools")}#BOXm
if(!require(car)){install.packages('car')}
if(!require(DescTools)){install.packages("DescTools")}

anova(lm(Y1~X1))
resulta <- aov(lm(Y1~X1))# used to summarize the analysis of variance model
summary(resulta)
...
# Homocedastico, caso valor nao seja significativo ou seja seja maior q 0.05
...
# y1 + y2 ~ x1
# X9 + X10 ~ X8
shapiro.test(hatco$X10[hatco$X8=="small"]) # considerar a normalidade de cada grupo somente
shapiro.test(hatco$X10[hatco$X8=="large"]) # considerar a normalidade de cada grupo somente
car::leveneTest(hatco$X10,hatco$X8) # Test for Homogeneity of Variance
#if(!require(mvoutlier)){install.packages('mvoutlier')}
par(mfrow=c(2,2));# alteracao da disposicao dos graficos
mvoutlier::aq.plot(hatco[c("X9",'X10')]); # nao tem outliers
# varios graus de criteriosidade, sendo poucos ignificativo, 
# pois apresentou alteracao em somente um grafico
par(mfrow=c(1,1));# alteracao da disposicao dos graficos

cor (hatco$X9, hatco$X10) # correlacao entre as variaveis dependentes
biotools::boxM(cbind(hatco$X9,hatco$X10), hatco$X8) # equivalencia das matrizes var-covar dos grupos
# significante caso seja abaixo de 5%, quando significante os as matrizes sao diferentes
summary(manova(lm(cbind(X9,X10) ~ X8, data=hatco)), test = 'Hotelling')
summary(manova(lm(cbind(X9,X10) ~ X8, data=hatco)), test = 'Pillai') # default
...
# X13	Type of industry-industry classification in which a product purchaser belongs.  This variable has two categories: 1 = industry A, 0 = other industries
View (hatco)
hatco$X13[hatco$X13==0]<-"other"; hatco$X13[hatco$X13==1]<-"industry"; 
hatco$X13<-as.factor(hatco$X13) # Categorizando a variavel resposta, qualitativa
table(hatco$X13, hatco$X8) # Balanceado!
# Two way ANOVA

summary.aov(lm(X9 ~ X8*X13, data=hatco)) # Significativo somente para X8, 
# summary.aov(lm(X9 ~ X8+X13+X8:X13, data=hatco)) # Mesma coisa
modelo <- aov(lm(X9 ~ X8, data=hatco)) # Teste Post HOC
TukeyHSD(modelo) # se desse ANOVA significativa para as outras variaveis dependentes, deriamos incluilas no modelo tb
# Como so tem dois niveis (large, small) no fator X8, nao tem o porque fazer o teste post hoc
# indicado se tivesse mais niveis, pois TukeyHSD faz teste par-a-par e  caso nao haveria com quem comparar
...
passo1<-lm(X9~X8*X13, data=hatco) # Intercept = 40.050 / X13other (coef Significativo)= 4.100
